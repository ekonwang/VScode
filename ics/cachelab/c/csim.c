...
int if_miss_S(long long addr, long long *ind, long long *p);
...
void cache_put(long long ind, long long p) {
    ...
    long long max = -1, min = 0x7fffffffffff, e_min;
    for(int i = 0; i < E; ++i) {
        long long val = CACHE[p][i].use_t;
        if(val > max) max = val;
        if(val < min) { min = val; e_min = i; }
    }
    CACHE[p][e_min].use_t = max+1;
    CACHE[p][e_min].ind = ind;
}
...
int if_miss(long long addr, long long *ind, long long *p) {
    ...
    *p = x_p;
    ...
    long long max = -1, lost = 1, id = -1;
    for(int i = 0; i < E; ++i) {
        long long u_t = CACHE[x_p][i].use_t; 
        max = max < u_t ? u_t : max;
        if( CACHE[x_p][i].ind == x_ind) {
            lost = 0;
            id = i;
        }
    }
    if(!lost) CACHE[x_p][id].use_t = max + 1;
    return lost;
}
